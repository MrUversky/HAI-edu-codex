...
Шаблон перехода:
1. `Сохранено:` назвать текущий утверждённый артефакт и путь к нему.
2. `Следующий этап:` назвать следующий шаг цепочки.
3. `Следующий артефакт:` назвать файл, который должен появиться после этого шага.

Не использовать расплывчатые фразы вроде:
- `сохрани и покажи следующий шаг`
- `идём дальше`
- `сейчас покажу, что дальше`

Переход должен явно разделять:
